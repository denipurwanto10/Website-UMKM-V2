...
@media (max-width: 768px) {
  ...
  .custom-pagination {
    ...
    <div class="container mt-3">
        ...
        <form action="<?php echo site_url('umkm/update1/'.$umkm['id']); ?>" method="post" enctype="multipart/form-data" onsubmit="return validateForm();">
...
<!-- UMKM Status -->
<div class="mb-3">
    <label style="color:white;" for="status" class="form-label"></label>
    <input type="hidden" name="status" value="<?= isset($umkm['status']) ? $umkm['status'] : 'menunggu'; ?>">
</div>
...
<script>
    // Mengambil status yang ada di input tersembunyi
    var status = '<?= isset($umkm['status']) ? $umkm['status'] : 'menunggu'; ?>';

    // Menampilkan catatan section hanya jika status adalah 'ditolak'
    if (status === 'ditolak') {
    ...
    // Jika status adalah 'ditolak', biarkan textarea bisa diubah, selain itu disabled agar tidak bisa diubah
    if (status !== 'ditolak') {
        document.getElementById('catatan').setAttribute('readonly', 'readonly'); // Membaca saja
    }
</script>
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Mengambil status dari PHP
        var status = '<?= isset($umkm['status']) ? $umkm['status'] : 'menunggu'; ?>';
        ...
        if (status === 'ditolak') {
            ...
            catatanTextarea.setAttribute('readonly', 'readonly'); // Membuat textarea readonly
        } else {
            catatanSection.style.display = 'none'; // Menyembunyikan catatan section jika status bukan 'ditolak'
        }
    });
</script>
<!-- JavaScript -->
