...
@media (max-width: 768px) {
  ...
  .custom-pagination {
    ...
    <?php include('sidebar.php'); ?>
        ...
        <form action="<?php echo site_url('umkm/store'); ?>" method="post" enctype="multipart/form-data" onsubmit="return validateForm();">
             <!-- Owner Name -->
 <div class="mb-3">
...
<div class="mb-3">
    ...
    <div class="form-check">
        ...
        <label style="color:white;" class="form-check-label" for="online">Online</label>
    </div>
    <div class="form-check">
        <input class="form-check-input" type="checkbox" id="offline" name="offline" value="Offline" 
        <?= set_value('offline') ? 'checked' : '' ?>>
        <label style="color:white;" class="form-check-label" for="offline">Offline</label>
    </div>
    <div class="form-check">
        ...
        <label style="color:white;" class="form-check-label" for="agen_reseller">Agen / Reseller</label>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    fetch('http://localhost:3000/api/users')
        ...
        .then(users => {
            ...
            // Tambahkan setiap user sebagai option
            users.forEach(user => {
                const option = document.createElement('option');
                option.value = user.username;  // Menggunakan username sebagai value
                option.textContent = user.username + " - " + user.fullname;  // Menampilkan username dan nama lengkap
                
                // Jika ingin mempertahankan nilai yang sudah dipilih sebelumnya
                if (option.value === "<?= set_value('username'); ?>") {
        ...
        .catch(error => {
            ...
            alert('Gagal memuat daftar pemilik. Silakan refresh halaman.');
        });
});
</script>
<!-- JavaScript -->
