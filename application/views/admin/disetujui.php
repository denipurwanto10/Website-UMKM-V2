...
<head>
    ...
    <title>Data UMKM - Disetujui</title>
    ...
    <style>
        .status-disetujui {
            background-color: rgba(0, 255, 0, 0.3); /* Hijau transparan */
        }
...
<div class="container mt-3">
    ...
    <div class="card">
        <div class="card-header  d-flex justify-content-between align-items-center">
            <h5 style="color:white;" class="mb-0">UMKM Disetujui</h5>
            <!-- Export Button for PDF -->
            <button id="exportPDFButton" class="btn btn-danger">
          ...
          <th style="text-align:center; color:white;" rowspan="2">Pendapatan</th> <!-- Alamat sub-column -->
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons" style="display: flex; gap: 10px;">
                                            ...
                                            <a href="<?= site_url('umkm/disetujui/delete/' . $user['id']); ?>" class="btn btn-sm btn-danger btn-delete">Hapus</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <?php else: ?>
                <div class="alert alert-warning text-center" role="alert">
                    Tidak ada data UMKM dengan status disetujui untuk ditampilkan.
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            document.getElementById('exportPDFButton').addEventListener('click', function() {
                var table = document.querySelector('#umkmTable');

                // Inisialisasi jsPDF
                const { jsPDF } = window.jspdf;
                const doc = new jsPDF();

                // Mengambil tanggal otomatis
                var today = new Date();
                var formattedDate = today.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });

                // Menentukan posisi tengah halaman
                var pageWidth = doc.internal.pageSize.width;

                // Menambahkan logo dan header
                var img = new Image();
                img.src = '<?php echo base_url("assets/img/logo_pemkab.png"); ?>'; // Sesuaikan lokasi logo

                img.onload = function() {
                    doc.addImage(img, 'PNG', 10, 10, 20, 20); // Posisi logo

                    // Header teks (ditengah) dengan font yang telah disesuaikan
                    doc.setFont('helvetica', 'bold'); // Arial tidak tersedia, helvetica mirip Arial
                    doc.setFontSize(12.5);
                    doc.text('PEMERINTAH KABUPATEN BANDUNG', pageWidth / 2, 15, { align: 'center' });

                    doc.setFontSize(14);
                    doc.text('DINAS PERDAGANGAN DAN PERINDUSTRIAN', pageWidth / 2, 22, { align: 'center' });

                    doc.setFont('helvetica', 'normal'); // Arial tidak tersedia, helvetica mirip Arial
                    doc.setFontSize(11);
                    doc.text('Jl. Raya Soreang Km. 17 Soreang 40912 Kabupaten Bandung Provinsi Jawa Barat', pageWidth / 2, 28, { align: 'center' });

                    doc.setFontSize(8.5);
                    doc.text('(022)-5894558 Email : disperindag@bandungkab.go.id Website : disperindag.bandungkab.go.id', pageWidth / 2, 34, { align: 'center' });
                    ...
                    // **Memperbaiki posisi tanggal agar tidak terlalu ke kanan**
                    doc.setFontSize(10);
                    doc.text(`Soreang, ${formattedDate}`, pageWidth - 70, 50); // Geser lebih ke kiri

                    // Adding the "Hal: Laporan Data UMKM" text below the date on the left
                    doc.setFontSize(10);
                    doc.text('Hal: Laporan Data UMKM', 10, 55); // Position it just below the date

                    // Extract rows data manually for PDF (group Alamat and Perijinan columns)
                    var rows = [];
                    ...
                    trs.forEach(function(tr) {
                        ...
                        tr.querySelectorAll('td').forEach(function(td, index) {
                            // Skip the first column (Photo)
                            if (index === 0) return;
                            
                            // For Alamat: separate Jalan, Kecamatan, Desa into different lines
                            if (index === 5) {
                                var jalan = td.textContent.trim();
                                var kecamatan = tr.cells[index + 1].textContent.trim();
                                var desa = tr.cells[index + 2].textContent.trim();
                                rowData.push(`Jalan: ${jalan}\nKecamatan: ${kecamatan}\nDesa: ${desa}`);
                            }
                            // For Perijinan: group NIB, PIRT, BPOM, Halal, HAKI, Lainnya
                            else if (index === 6) {
                                rowData.push('NIB: ' + td.textContent.trim());
                            } else if (index === 7) {
                                rowData.push('PIRT: ' + td.textContent.trim());
                            } else if (index === 8) {
                                rowData.push('BPOM: ' + td.textContent.trim());
                            } else if (index === 9) {
                                rowData.push('Halal: ' + td.textContent.trim());
                            } else if (index === 10) {
                                rowData.push('HAKI: ' + td.textContent.trim());
                            } else if (index === 11) {
                                rowData.push('Lainnya: ' + td.textContent.trim());
                            } else if (index !== 12) { // Skip the last column (Aksi)
                                rowData.push(td.textContent.trim());
                            }
                        });

                        // Combine all the "Perijinan" data into one cell
                        var perijinan = rowData.slice(5, 11).join("\n"); // Perijinan related values
                        rowData[5] = perijinan; // Place the combined "Perijinan" in the Perijinan column
                        rows.push(rowData);
                    });

                    // Define table columns (with "Alamat" and "Perijinan" as a single column each)
                    var columns = [
                        'Nama Pemilik', 'Nama Usaha', 'Merek', 'Kategori', 'Alamat', 'Perijinan'
                    ];

                    // Adding table to PDF
                    doc.autoTable({
                        head: [columns],  // Column headers
                        body: rows,       // Rows of data
                        startY: 60, 
                        margin: { horizontal: 10 },
                        styles: { 
                            font: 'helvetica', 
                            halign: 'left',  // Left align body content (Alamat and Perijinan)
                            lineWidth: 0.5,
                            minCellHeight: 8, // Ensure the minimum height for each row
                        },
                        headStyles: {
                            halign: 'center',  // Center align the headers
                            font: 'helvetica',
                            fontSize: 10,
                            lineWidth: 0.5,
                            fillColor: [000, 000, 000], // Light grey header background
                        },
                        bodyStyles: {
                            halign: 'left',  // Keep body content left aligned
                            valign: 'top',   // Align text to the top of the cell to avoid overflow
                        },
                        columnStyles: {
                            0: { cellWidth: 'auto' }, // Adjust the first column to auto width
                            1: { cellWidth: 'auto' }, // Adjust the second column to auto width
                            2: { cellWidth: 'auto' }, // Adjust the third column to auto width
                            3: { cellWidth: 'auto' }, // Adjust the fourth column to auto width
                            4: { cellWidth: 'auto' }, // Adjust the fifth column to auto width
                            5: { cellWidth: 'auto' }, // Adjust the "Perijinan" column to auto width
                        }
                    });

                    // Save the PDF
                    doc.save(`UMKM_Laporan_Disetujui_${today.getFullYear()}-${today.getMonth()+1}-${today.getDate()}.pdf`);
                };
            });
        });
    </script>
